...
class StudentUSocket(StudentUSocketBase):
  ...
  def handle_accepted_payload(self, payload):
    ...
    self.rcv.wnd = self.rcv.wnd -  len(payload)
  ...
  def handle_accepted_fin(self, seg):
    ...
    ## Start of Stage 7.2 ##
    if self.state is FIN_WAIT_1 and seg.FIN and seg.ACK:
        self.set_pending_ack()
        self.start_timer_timewait()
    elif self.state is FIN_WAIT_1 and seg.FIN and not seg.ACK:
         self.set_pending_ack()
         self.state  = CLOSING
    elif self.state is FIN_WAIT_2 and seg.FIN:
